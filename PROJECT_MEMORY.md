...
- Superseded below: the extra service token requirement was an implementation mistake. User clarified buyers should need only their own AppID/AppSecret and the shared server IP whitelist.
...
## 2026-09-16: Buyer calls require only their own WeChat credentials

- User clarified there must be no separate service-access credential. Removed API_TOKEN startup/auth requirement in request mode, and removed buyer Authorization headers and mandatory token config from Markdown/HTML/newspic clients. Existing legacy service authentication remains unchanged.
- Buyer setup: whitelist 8.153.207.214; keep their own AppID/AppSecret locally; use the bundled HTTPS service URL. Each request is authenticated with WeChat using those credentials.
- Validation: 34 Python tests and 8 Node HTTP integration tests passed, including exact production Node18.20.8. Public credential-only smoke passed 11 checks: token connection, missing/invalid credential rejection, body/permanent-image uploads, image download/decode, article create/update/readback/editor compatibility, picture create/update/reorder/readback. Drafts only, no public publication.
- Deployed isolated wechat-skill-api release6bfafde, source SHA2566bfafde556a264e4c4d4fd6289964041bbcc877379a8a0278f3e0c7c4df377b2. PM2 online, internal/public health report request mode and authentication=wechat_credentials. Legacy source SHA256366304b5b33f1cc1a209e6017d2bddefdf9b3b7b3295e5080f365c7eb6eabc52 unchanged and healthy. PM2 state saved.
- Local buyer configuration path: ~/.codex/secrets/wechat_mp_buyer.env (600); variables WECHAT_MP_APP_ID, WECHAT_MP_APP_SECRET, WECHAT_MP_API_BASE_URL, WECHAT_MP_CREDENTIAL_MODE. No extra token. Live evidence: /Users/burning/Documents/workbuddy/outputs/wechat-no-service-token-20260916/live/.
- Replacement sale artifact: /Users/burning/Documents/workbuddy/outputs/wechat-skillpay-20260916/release-buyer-credentials-only/wechat-content-studio-1.0.0.zip (28082bytes,11files); SHA2563e2bc17e1706d38807eb01fdf0aab57ec6ed793faebd0f81bf5e75b84af44f1d. ZIP integrity, actual-secret exclusion, extracted CLI help and packaged-client public authentication without service token passed.
- SkillPay form remains not uploaded/submitted because supported file upload was denied by Chrome extension file URL permissions. Buyer credential-delivery blocker is resolved. Preserve user's revised 291-character description, price9.99 and buyout setting. Upload the replacement artifact after user enables file access.
