...
def main():
    ...
    parser.add_argument('--label', default='公网', help='Short label distinguishing live test drafts')
    ...
    token=''  # Buyer calls use only AppID/AppSecret.
    out=Path(args.output).resolve();out.mkdir(parents=True,exist_ok=True)
    paths=[Path(x).resolve() for x in args.images]
    images=newspic.prepare(f'{args.label}贴图实测', '公网贴图测试：两张图片按输入顺序展示。',paths)
    run_hash=newspic.fingerprint(base,appid,f'public-smoke-v2:{args.label}','',images)
    ...
    try:
        ...
        for name,extra in [('missing_credentials',{}),('invalid_credentials',{'X-Wechat-Appid':'wx'+'0'*16,'X-Wechat-Appsecret':'0'*32})]:
            req=urllib.request.Request(base+'/token/test',data=b'{}',method='POST',headers={'Content-Type':'application/json',**extra})
            try:
        ...
        article={'article_type':'news','title':f'{args.label}文章实测｜燃烧青年','author':'燃烧青年','content':content,'thumb_media_id':cover['media_id'],'need_open_comment':0,'only_fans_can_comment':0}
        ...
        changed={**article,'title':f'{args.label}文章实测｜已更新','content':content+'<p>草稿更新验证完成。</p>'}
        ...
        if 'picture_created' not in state['results']:
            pic=newspic.publish(publish.api_post,base,token,'appid-sha256:'+hashlib.sha256(appid.encode()).hexdigest(),f'{args.label}贴图实测','公网贴图测试：两张图片按输入顺序展示。',images,pic_receipt)
        ...
        changed_pic={'article_type':'newspic','title':f'{args.label}贴图实测｜已更新','content':'更新验证：两张图片顺序已交换。','need_open_comment':0,'only_fans_can_comment':0,'image_info':{'image_list':[{'image_media_id':mid} for mid in updated_ids]}}
