#!/usr/bin/env bash
#
# evolink-image-gen.sh — Generate AI images via EvoLink API (Nano Banana 2)
#
# Usage:
...
#   ./evolink-image-gen.sh "Coffee shop logo" --n 4
#   ./evolink-image-gen.sh "Abstract pattern" --model nano-banana-2-lite
...
for ((i=1; i<=MAX_POLL_ATTEMPTS; i++)); do
  ...
  case "$STATUS" in
    completed|success)
      URLS=$(echo "$POLL_RESPONSE" | jq -r '.result_urls // .results // .urls // .data // empty | if type == "array" then .[] else . end' 2>/dev/null)
      echo ""
